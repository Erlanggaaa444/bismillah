    ...
    bool hasTicked()
    {
      if (elapsedTime > 0 && millis() - previousMillis >= elapsedTime)
      {
        ...
        if (countdownTime <= 0) // If countdown time is down to 0
        {
          ...
          return true; // Return true only when countdown time is down to 0
        }
      }
      return false;
    }
...
void listenmode0()
{
  int elapseTimeSet = readKp4x4_set_timer();

  if(elapseTimeSet > 0){
  timerValue = inputString.toFloat();
  timer.setElapsedTime(timerValue);
  }
