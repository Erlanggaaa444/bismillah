// deklarasikan kelas modul relay
class RelayModule
{
  private:
    int relayPins[8];
...
char checkInterrupt() {
  ...
  if (Serial.available()) {
    input = Serial.read();
    if (input == '*') { // If '*' character is received
      mode = 0;
    }
...
void display_set_threshold(int sensor_value)
{
  // display lcd
  lcd.clear();
  lcd.setCursor(0, 0);
...
void listenmode1()
{
  ...
  while (true)
  {
    ...
    Serial.print("threshold diatur di ");
...
{
  int sensorValue = analogRead(pinSensor);
  if (sensorValue < moistureThreshold - 20)
  {
    relay.turnOnSpecific(selectedRelay);
  }
  else
